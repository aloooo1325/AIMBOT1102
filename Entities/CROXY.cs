...
using System.Text;
using System.Threading.Tasks;                  //private static INTERNAL AYUSH = new INTERNAL();
    ...
    public class INTERNAL
    {
        ...
        public Task<IEnumerable<long>> AoBScan(string search, bool writable = true, bool executable = false, string file = "")
        {
            return Task.Run(() =>
            {
                ...
                while (currentAddr.ToUInt64() < maxEnd)
                {
                    ...
                    // 2. Filter Regions (CRITICAL FOR SPEED)
                    // Only scan COMMITTED memory. 
                    // Usually games store data in PRIVATE memory (Type == 0x20000).
                    bool isCommit = mem.State == MEM_COMMIT;
                    bool isAccessible = (mem.Protect & PAGE_NOACCESS) == 0 && (mem.Protect & 0x100) == 0; // No Guard

                    if (isCommit && isAccessible)
                    {
                        bool isWritable = (mem.Protect & 0x04) > 0 || (mem.Protect & 0x40) > 0; // ReadWrite or ExecuteReadWrite
                ...
                // 3. Parallel Scan with Unsafe Pointers
                    ...
                    if (ReadProcessMemory(pHandle, region.BaseAddress, buffer, (UIntPtr)region.RegionSize, IntPtr.Zero))
                    {
                        unsafe
                        {
                            ...
                            fixed (byte* pMask = pattern.Mask)
                            {
                                ...
                                {
                                    // Optimization: Check first byte before looping
        ...
        public bool WriteMemory(string address, string type, string value, string offset = "")
        {
            try
            {
                ...
                // Handle optional offset if needed, usually passed as hex string
            ...
            public int ReadInt(long address)
        {
            try
            {
                ...
                // Reads 4 bytes (Integer) directly from memory
